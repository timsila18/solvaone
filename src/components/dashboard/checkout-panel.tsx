...
export function CheckoutPanel({ projectId, productId, initialPaymentId = null, initialStatus = "pending", initialMessage = null }: CheckoutPanelProps) {
  ...
  const paymentConfirmed = status === "successful" || status === "paid";
  ...
  return (
    <section className="rounded-lg border border-black/10 p-5 dark:border-white/10">
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-[1fr_auto]">
        ...
        <Button onClick={initiatePayment} disabled={isPending || status === "processing" || paymentConfirmed || !phone}>
      ...
      <div className="mt-5 rounded-lg border border-black/10 p-4 dark:border-white/10">
        ...
        {(status === "failed" || status === "cancelled" || status === "timed_out") && (
          <div className="mt-4 flex flex-wrap gap-2">
            {paymentId ? (
              <Button variant="secondary" onClick={refreshPaymentStatus} disabled={isPending}>
                <RefreshCw className="h-4 w-4" /> Check payment status
              </Button>
            ) : null}
            <Button variant="secondary" onClick={initiatePayment} disabled={isPending || !phone}>
              <Phone className="h-4 w-4" /> Retry STK Push
            </Button>
          </div>
        )}
        {paymentConfirmed && (
